@props(['id', 'title', 'message', 'confirmText' => 'Confirmer', 'cancelText' => 'Annuler', 'action' => null, 'method' => 'POST'])

<div x-data="{ open: false, currentAction: '{{ $action }}' }" 
     x-show="open" 
     @open-modal-{{ $id }}.window="open = true"
     @close-modal-{{ $id }}.window="open = false"
     @set-action-{{ $id }}.window="currentAction = $event.detail"
        ...
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <div class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                ...
                <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                    <form method="{{ $method === 'GET' ? 'GET' : 'POST' }}" :action="currentAction">
                        @csrf
